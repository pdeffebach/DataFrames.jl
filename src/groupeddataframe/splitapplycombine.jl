...
be slower than using the equivalent `GroupKey`. Finally, a `Dict` with
column names as keys can be used (order does not matter in this case).
Note that indexing with `GroupKey` is faster than other methods.
